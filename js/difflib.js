...
difflib={defaultJunkFunction:function(c){return c in __whitespace;}
...
return lines;
    ...
    if(initial!=null){var value=initial;var idx=0;
        }else if(list){
            var value=list[0];
            var idx=1;
        }else
...
this.set_seq2=function(b){
    if(b==this.b)return;
    this.b=b;this.matching_blocks=this.opcodes=this.fullbcount=null;
    this.__chain_b();
}
    ...
    for(var i=0;i<b.length;i++){
        ...
        if(elt in b2j){
            ...
            if(n>=200&&indices.length*100>n){
                populardict[elt]=1;delete b2j[elt];
            }else{
                indices.push(i);
            }
        }
...
this.get_opcodes=function(){
    ...
    for(var idx in blocks){block=blocks[idx];
        ai=block[0];bj=block[1];size=block[2];
        tag='';
        if(i<ai&&j<bj){
            tag='replace';
        }else if(i<ai){
...
var nn=n+n;
var groups=[];
for(var idx in codes){code=codes[idx];
    tag=code[0];
    i1=code[1];
    i2=code[2];
    j1=code[3];
...
this.real_quick_ratio=function(){
    var la=this.a.length;var lb=this.b.length;
